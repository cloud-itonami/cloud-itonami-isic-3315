(ns transport-equipment-repair.notify
  ...
  the transport-equipment-repair-domain analog of `fabricated-metal-
  repair.notify`/`installation.notify`/`demolition.notify`/`electrical-
  equipment-repair.notify`/`other-equipment-repair.notify`.
  ...
  SAME notice out to every contact in an asset's `:safety-contacts`
  roster (the repair technician / shop safety officer, NOT the customer
  -- this actor coordinates internal repair-shop safety response, it
  does not manage customer communications) over BOTH channels, isolating
  one contact's send failure from every other contact's.

  This repo deliberately ships NO JVM-only interop anywhere in `src/` --
  see this workspace's cljs-first `.cljc` runtime-priority rule
  (CLAUDE.md `.cljc`/`.kotoba` runtime priority section: `kotoba wasm` >
  `clojurewasm` > `ClojureScript` > `nbb`, JVM/`bb` downgraded to a last
  resort) and this build's explicit no-JVM-interop mandate. A real
  transport (Resend mail / Twilio voice, or an equivalent) can be added
  later behind the SAME `Notifier` protocol via a portable HTTP client
  (e.g. a `cljs`/`nbb` `fetch`-based implementation) without changing
  this namespace's shape or any caller -- until then, this actor's
  safety-concern notice is 'sent' only via the deterministic mock, and
  callers that need a real send must supply their own `Notifier`
  implementation.

  `:flag-safety-concern` is NEVER fired by an auto-commit -- it always
  escalates to a human first (see `transport-equipment-repair.phase`/
  `transport-equipment-repair.governor` ns docstrings), so by the time
  `transport-equipment-repair.operation`'s `:commit` node calls this, a
  human has already reviewed the concern. This namespace builds/sends
  the message; it does NOT decide whether to send one."
  )
...
(defn dispatch-safety-concern-notice!
  ...
  every other responsible party of a flagged structural-integrity/
  airworthiness/seaworthiness/rail-safety concern."
