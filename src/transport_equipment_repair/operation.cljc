(ns transport-equipment-repair.operation
  "OperationActor -- one transport-equipment-repair-shop coordination
  operation = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (Repair Advisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Repair Governor
  ...
  rewrite:
    ...
    - the Notifier (mock -- the only implementation this repo ships)       - :notifier opt
    - the Phase    (0->3 rollout)                                         - :phase in ctx

  One graph run = one repair-shop-coordination operation (intake ->
  ...
  decision to a human operator (the qualified repair-shop operator). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:flag-safety-concern` ALWAYS reaches this node -- see
  `transport-equipment-repair.phase`/`transport-equipment-repair.
  governor` ns docstrings. `:log-repair-record`, `:schedule-repair-
  operation` (when governor-clean) and `:coordinate-return-to-service`
  (when governor-clean) MAY auto-commit at phase 3 -- see `transport-
  equipment-repair.phase` ns docstring.

  Every committed record carries `:effect :propose` (see `transport-
  equipment-repair.governor` ns docstring) -- the `:commit` node writes a
  COORDINATION ARTIFACT to the SSoT + audit ledger, NEVER a real-world
  actuation. ONLY when the committed op is `:flag-safety-concern` does
  `:commit` ADDITIONALLY send the safety-concern notice (mail + phone,
  `transport-equipment-repair.notify`) to the asset's `:safety-contacts`
  roster -- always AFTER a human has approved it, since `:flag-safety-
  concern` is never auto-eligible at any phase."
...
(defn build
  ...
  `transport-equipment-repair.store/Store`).
  opts:
    :advisor      -- a `transport-equipment-repair.advisor/Advisor` (default: mock-advisor)
    :notifier     -- a `transport-equipment-repair.notify/Notifier` (default: mock-notifier)
  ...
  (-> (g/state-graph
      ...
      ;; Repair Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Repair Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
        ...
        (fn [{:keys [request context proposal record]}]
          ...
          (let [f (commit-fact request context proposal)
                notify-result (when (= :flag-safety-concern (:op record))
                                (let [a (store/asset store (:subject request))]
                                  (notify/dispatch-safety-concern-notice!
                                   notifier (:safety-contacts a) (:value proposal))))]
