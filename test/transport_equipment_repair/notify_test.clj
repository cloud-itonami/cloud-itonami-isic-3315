(ns transport-equipment-repair.notify-test
  "Tests the mock `Notifier` + `dual-notifier` composition seam. This
  repo ships no real (JVM-only) Resend/Twilio transport to stub-and-test
  -- see `transport-equipment-repair.notify` ns docstring."
...
(deftest dispatch-safety-concern-notice-fans-out-to-every-contact-both-channels
  (let [n (notify/mock-notifier)
        contacts [{:name "Hayato" :email "hayato@example.com" :phone "+819000000041"}
                  {:name "Nozomi" :email "nozomi@example.com" :phone "+819000000042"}]
